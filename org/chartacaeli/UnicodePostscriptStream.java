	...
	public UnicodePostscriptStream( OutputStream out ) {
		...
		try {
			...
			if ( Preferences.systemRoot().nodeExists( name ) ) {
				...
				for ( String child : snode.childrenNames() )
					if ( child.matches( "[0-9A-Fa-f]{4,6}\\.\\.[0-9A-Fa-f]{4,6}-[0-9]+" ) ) {
						fontname = snode.node( child ).get( CK_FONTNAME, null ) ;
						encoding = snode.node( child ).get( CK_ENCODING, null ) ;
			...
			if ( Preferences.userRoot().nodeExists( name ) ) {
				...
				for ( String child : unode.childrenNames() )
					if ( child.matches( "[0-9A-Fa-f]{4,6}\\.\\.[0-9A-Fa-f]{4,6}-[0-9]+" ) ) {
						fontname = unode.node( child ).get( CK_FONTNAME, null ) ;
						encoding = unode.node( child ).get( CK_ENCODING, null ) ;
						if ( fontname == null || encoding == null )
							continue ;
						addUnicodeControlBlock( child, fontname, encoding ) ;
					}
